    ...
    NewMaterialUData * udata)
{
  ...
  if (udata->async == FALSE) {
    udata->material = material;
    ...
    GstClockTime duration, GESTrackType track_types)
{
  ...
  if (GST_CLOCK_TIME_IS_VALID (duration)) {
    ges_timeline_object_set_duration (tlobj, inpoint);
  }
