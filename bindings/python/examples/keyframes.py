...
class Effect:
    def __init__(self, effects):
        ...
        self.src.set_vpattern("smpte")
        layer.add_object(self.src)
        self.timeline.add_layer(layer)

        self.add_effects(effects)

        element = self.get_element_from_effect(self._effects[0], "videobalance0")
        self.controller = gst.Controller(element, "hue")
        self.controller.set_interpolation_mode("hue", gst.INTERPOLATE_CUBIC)
    ...
    def get_element_from_effect(self, effect, name):
        for element in list(effect.get_element().elements()):
